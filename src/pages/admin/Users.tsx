  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <h2 className="text-lg font-semibold text-stone-900">ผู้ใช้งาน</h2>
          <p className="text-sm text-stone-400">จัดการบัญชีผู้ใช้งานและสิทธิ์การเข้าถึงระบบ</p>
        </div>
          ...
          {ROLES.map((r) => (
            <button
              ...
              className={`rounded-full px-3 py-1 text-xs font-medium transition ${
                role === r ? 'bg-emerald-500 text-white shadow-sm' : 'text-stone-500 hover:text-emerald-700'
              }`}
            >
      ...
      {filtered.length === 0 ? (
        ...
        <div className="overflow-x-auto rounded-2xl border border-stone-100 bg-white">
          <table className="min-w-[720px] w-full divide-y divide-stone-100 text-sm">
            ...
            <tbody className="divide-y divide-stone-50">
              {filtered.map((u) => (
                <tr key={u.id} className="hover:bg-emerald-50/30">
                  <td className="px-4 py-2.5 font-medium text-stone-900">{u.name}</td>
                  <td className="px-4 py-2.5 text-stone-500">{roleLabel[u.role]}</td>
                  <td className="px-4 py-2.5 text-stone-500">{u.floor === 'all' ? 'ทุกชั้น' : `ชั้น ${u.floor}`}</td>
                  <td className="px-4 py-2.5 text-stone-500">{u.lastLogin}</td>
