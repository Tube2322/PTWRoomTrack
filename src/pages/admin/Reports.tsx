  ...
  return (
    <div className="space-y-8">
      <div>
        ...
        <p className="text-sm text-gray-400">สรุป KPI และผลการทำห้องรายชั้น (ข้อมูลตัวอย่าง)</p>
      </div>
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
        ...
        <div className="rounded-2xl border border-gray-100 bg-white p-4">
          ...
          <p className="text-xs text-gray-400">จำนวนห้องรอซ่อม</p>
        </div>
      </div>

      <section>
        <h3 className="mb-3 text-sm font-semibold text-gray-500">เปอร์เซ็นต์ผ่านเป้าหมาย 20 นาที แยกตามชั้น</h3>
        <div className="space-y-4 rounded-2xl border border-gray-100 bg-white p-5">
          {FLOORS.map((f) => (
            <div key={f}>
              <div className="mb-1.5 flex items-center justify-between text-xs text-gray-500">
                <span className="font-medium text-gray-700">ชั้น {f}</span>
      ...
      <div className="flex justify-end">
        <button className="rounded-full border border-gray-200 px-4 py-2 text-sm font-medium text-gray-600 transition hover:border-emerald-300 hover:text-emerald-700">
          ส่งออกรายงาน (CSV)
        </button>
      </div>
    </div>
